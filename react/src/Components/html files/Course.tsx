  ...
  useEffect(() => {
    ...
    const getLessons = async () => {
      try {
        ...
        console.log("User data:", data.lessons);
      } catch (err) {
                        ...
                        {activeVideo.unitIndex === unitIndex && activeVideo.lessonIndex === lessonIndex && (
                          <div className="mt-4">
                            <iframe
                              width="100%"
                              height="315"
                              src={lesson?.videoUrls?.[0]?.url ?? "https://res.cloudinary.com/dfpx3gupk/video/upload/v1746475198/lessons/lesson3/qri9srse13e1a4pr9vrm.mp4"}
                          ...
                          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {lesson?.imageUrls?.map((img, i) => (
                              <div
                                ...
                                onClick={() => setActiveImage({ unitIndex, lessonIndex })}
                              >
                                <img
                                  src={img.url}
                                  alt={`Lesson Image ${i + 1}`}
                                  className={`w-full h-auto rounded shadow ${activeImage.unitIndex === unitIndex && activeImage.lessonIndex === lessonIndex && activeImage.lessonIndex === i ? 'border-4 border-blue-500' : ''}`}
                                />
                              </div>
                            ))}
                          </div>
